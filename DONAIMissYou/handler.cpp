                             ...
                             qreal theta, qreal psi, qreal phi)
{
    ...
    double voltageNum = (voltage.toDouble()/250)*294.3*5.33*0.975*1000; //convert voltage sent by cody to torque, it says voltageNum but it is a torque, sorry
    ...
    torqueStr = QString::number(torqueFlt);                             //values are stored here to be pulled and added to list when timer goes off. These change with every socket

    emit homeDisplay(torqueFlt, tMax, units);
...
{
    subVal = zeroVoltage.toDouble();                    //this stores the current torque so it can be subtracted from new incoming torques this gives a zero baseline (taring)
}
